...
import {
    Container, Col, Row, Form, Table, Modal, Button, ToggleButton, ButtonGroup, ButtonToolbar, Dropdown, Alert
} from "react-bootstrap";
import DropdownToggle from "react-bootstrap/DropdownToggle";

function App() {
    const [dockedData, getDockedData] = useState([])
    const dockedURL = 'http://' + process.env.REACT_APP_BACKEND_SERVICE_ADDRESS + '/api/list/0';

    const [outboundData, getOutboundData] = useState([])
    const outboundURL = 'http://' + process.env.REACT_APP_BACKEND_SERVICE_ADDRESS + '/api/list/1';

    const [inboundData, getInboundData] = useState([])
    const inboundURL = 'http://' + process.env.REACT_APP_BACKEND_SERVICE_ADDRESS + '/api/list/2';

    const [maintenanceData, getMaintenanceData] = useState([])
    const maintenanceURL = 'http://' + process.env.REACT_APP_BACKEND_SERVICE_ADDRESS + '/api/list/3';
    ...
    const radios = [{name: 'Docked', value: '0'}, {name: 'Outbound', value: '1'}, {
        name: 'Inbound',
        value: '2'
    }, {name: 'Maintenance', value: '3'}];

    function deleteData() {
        fetch('http://' + process.env.REACT_APP_BACKEND_SERVICE_ADDRESS + '/api/deleteboat/' + selectedData.id, {
            method: "DELETE"
        })
            //.then(resp => resp.json())
            .then(() => updateTables())
    ...
    function editData() {
        fetch('http://' + process.env.REACT_APP_BACKEND_SERVICE_ADDRESS + '/api/editboat/' + selectedData.id, {
            method: "POST", headers: {
    ...
    function addData(targetData) {
        ...
        fetch('http://' + process.env.REACT_APP_BACKEND_SERVICE_ADDRESS + '/api/addboat', {
            method: "POST", headers: {
                "Content-Type": "application/json"
